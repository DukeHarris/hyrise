...
std::vector<int> scannedFields;
  ...
  std::cout << "Fields: " << scannedFields.at(0) << std::endl;
  idx_scan->recordColumnScan(data["table"].asString(), scannedFields);
...
void CompoundIndexScan::recordColumnScan(std::string table, std::vector<int> fields){
...
std::vector<int> CompoundIndexScan::getScannedColumns(){
